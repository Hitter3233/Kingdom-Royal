...
let units=[],particles=[],mana=5,time=180,selected=null,gameOver=false,last=performance.now(),aiThink=1.5,effects=[],floaters=[];
const towers=[
...
function start(){units=[];particles=[];effects=[];mana=5;time=180;selected=null;gameOver=false;DeckSystem.setup();UI.renderHand();last=performance.now();requestAnimationFrame(loop)}
