...
function drawUnit(u){
 ...
 // shadow
 ctx.fillStyle="#0008";ctx.beginPath();ctx.ellipse(0,22,21,8,0,0,Math.PI*2);ctx.fill();
 // aura ring
 ctx.strokeStyle=u.team==="player"?"#78bfff55":"#ff767655";ctx.lineWidth=2;
 ctx.beginPath();ctx.arc(0,0,23+Math.sin(u.age*4)*2,0,Math.PI*2);ctx.stroke();
 // body
 ctx.fillStyle=u.color;ctx.strokeStyle=u.team==="player"?"#8dccff":"#ff9b9b";ctx.lineWidth=3;
 ctx.beginPath();ctx.arc(0,0,18,0,Math.PI*2);ctx.fill();ctx.stroke();
 // head/helmet highlight
 ctx.fillStyle="#ffffff55";ctx.beginPath();ctx.arc(-5,-7,7,0,Math.PI*2);ctx.fill();
 // cross / class symbol
 ctx.fillStyle="#fff";ctx.font="bold 17px serif";ctx.textAlign="center";
 ctx.fillText(u.name==="Light"?"✦":"✝",0,6);
 // tiny team marker
 ctx.fillStyle=u.team==="player"?"#bfe4ff":"#ffd0d0";ctx.beginPath();ctx.arc(0,23,3,0,Math.PI*2);ctx.fill();
 ctx.restore();
 hpbar(u.x,u.y-31,50,6,u.hp,u.max);
 ctx.fillStyle="#fff";ctx.font="bold 10px system-ui";ctx.textAlign="center";
 ctx.shadowColor="#000";ctx.shadowBlur=3;ctx.fillText(u.name,u.x,u.y-39);ctx.shadowBlur=0;
}
