...
let units=[],particles=[],mana=5,time=180,selected=null,gameOver=false,last=performance.now(),aiThink=2,effects=[];
const towers=[
...
function cardArrival(x,y,color){for(let i=0;i<16;i++)effects.push({x,y,r:4+Math.random()*12,life:.65,max:.65,color});}
function drawEffects(dt){effects.forEach(e=>{e.life-=dt;e.r+=35*dt;ctx.globalAlpha=Math.max(0,e.life/e.max);ctx.strokeStyle=e.color;ctx.lineWidth=3;ctx.beginPath();ctx.arc(e.x,e.y,e.r,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1});effects=effects.filter(e=>e.life>0);}
...
function deploy(x,y){
 ...
 mana-=card.cost;
const lane=x<500?0:1;
spawn(card,"player",lane);
cardArrival(laneX[lane],600,"#ffe27a");
const playedIndex=selected;
DeckSystem.play(selected);
selected=null;
UI.renderHand();
UI.animateReplacement(playedIndex);
}
